...
if ($ReadOnlyServer) { [IO.File]::WriteAllText((Join-Path $OutputDir 'claude-settings.json'),"{`"enabledPlugins`":{},`"hooks`":{},`"permissions`":{`"allow`":[],`"deny`":[]},`"mcpServers`":$servers}`n",$Utf8) } else { Copy-Item -LiteralPath $claudeTemplate -Destination (Join-Path $OutputDir 'claude-settings.json') -Force }
