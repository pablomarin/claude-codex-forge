...
run_guarded_dispatch() {
    ...
    mkdir -p "$primary" "$candidate" "$scratch/home" "$scratch/session-store" "$scratch/codex-home" "$replay"
    git -C "$primary" init -q; seed_frozen_candidate "$candidate"
    printf 'FORGE_CANARY_USER_INSTRUCTION\n' > "$scratch/home/CLAUDE.md"
    ...
    if [ "$engine" = claude ]; then
        (cd "$primary" && HOME="$scratch/home" FORGE_DISPATCH_SENTINEL="$sentinel" "$binary" -p --safe-mode --no-session-persistence --strict-mcp-config --mcp-config "$empty_mcp" --setting-sources '' --tools '' --permission-mode dontAsk --output-format json --system-prompt "Return sentinel=$sentinel and canary_observed=false." "$sentinel") > "$ephemeral_out" 2> "$scratch/ephemeral.err" || { reason="Claude ephemeral isolation invocation failed"; return 1; }
    else
    ...
    if [ "$engine" = claude ]; then
        (cd "$primary" && HOME="$scratch/home" FORGE_DISPATCH_SESSION_ID="$session" FORGE_DISPATCH_SEAT_HASH="$seat" FORGE_DISPATCH_CONFIG_HASH="$config" "$binary" -p --safe-mode --strict-mcp-config --mcp-config "$empty_mcp" --setting-sources '' --tools '' --permission-mode dontAsk --output-format json --session-id "$session" --system-prompt "FORGE_COUNCIL_START session_id=$session seat_hash=$seat config_hash=$config canary_observed=false" "FORGE_COUNCIL_START") > "$start_out" 2> "$scratch/start.err" || { reason="Claude council first turn failed"; return 1; }
        validate_bound_response "$start_out" "$session" "$seat" "$config" || { reason="Claude council first turn identity mismatch"; return 1; }
        (cd "$primary" && HOME="$scratch/home" FORGE_DISPATCH_SESSION_ID="$session" FORGE_DISPATCH_SEAT_HASH="$seat" FORGE_DISPATCH_CONFIG_HASH="$config" "$binary" -p --safe-mode --strict-mcp-config --mcp-config "$empty_mcp" --setting-sources '' --tools '' --permission-mode dontAsk --output-format json --resume "$session" --system-prompt "FORGE_COUNCIL_RESUME exact session_id=$session seat_hash=$seat config_hash=$config canary_observed=false" "FORGE_COUNCIL_RESUME") > "$resume_out" 2> "$scratch/resume.err" || { reason="Claude exact-id resume failed"; return 1; }
    else
    ...
    cp -R "$candidate" "$investigation"
    if [ "$engine" = claude ]; then
        (cd "$investigation" && HOME="$scratch/home" FORGE_DISPATCH_INVESTIGATION_ROOT="$investigation" FORGE_DISPATCH_REPLAY_TARGET="$replay" FORGE_DISPATCH_CANDIDATE_ID="$before" "$binary" -p --safe-mode --no-session-persistence --strict-mcp-config --mcp-config "$empty_mcp" --setting-sources '' --tools 'Read,Write,Edit,Bash' --permission-mode dontAsk --output-format json --system-prompt "FORGE_INVESTIGATION candidate_id=$before. Write only artifacts/qualification.txt, text <=65536 bytes. Respond candidate_id=$before and canary_observed=false." "FORGE_INVESTIGATION") > "$investigation_out" 2> "$scratch/investigation.err" || { reason="Claude investigation invocation failed"; return 1; }
    else
