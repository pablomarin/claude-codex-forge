...
$agent = Join-Path $self 'agent-dispatch.ps1'; $hostContext = Join-Path $self 'host-context.ps1'; $capabilities = Join-Path $root 'manifests\host-capabilities.tsv'
...
function Invoke-Seat([string]$Dir,[string]$Seat,[string]$Phase,[string]$Bundle='',[string]$Session='') {
  ...
  if ($Phase -eq 'peer') {$args += @('--session-id',$Session)} else {$args += @('--session-id-output',(Join-Path $Dir "$Seat.session"))}; & $agent @args; return $LASTEXITCODE
}
