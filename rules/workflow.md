...
**Never Bash-read `.claude/local/state.md`.** Read it with the **Read tool**; write/update it with **Edit/Write** (auto-approved by `auto-approve-local-writes`). Reading state.md through a Bash read-utility (`cat`/`sed`/`grep`/`awk`/`head`/`tail`/`rg`/…) trips Claude Code's sensitive-file access prompt — Bash is not a read-only tool — and in a `/goal` loop with no human that silently stalls the run, the same failure mode as ask-tier commands. The Read tool is prompt-free for in-project files. The `check-bash-safety` hook blocks a read-utility that targets `.claude/local/state.md` inline and points you back to the Read tool; it is a **targeted guardrail for the common inline form**, not a guarantee (variable-indirected reads like `sed "$STATE"` are intentionally allowed because the sanctioned `extract_foldable` seed/fold-back blocks rely on them — those read via a shell variable, so the utility token and the literal path never share a line). The hook gates the **agent's Bash tool**, not a human's terminal — a person inspecting state.md with `cat` in their own shell is unaffected.
