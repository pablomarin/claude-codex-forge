...
**Phase-gate fresh-session selection no longer pre-approves implementation.** The runtime controller now separates gate mode selection from approval: `select-gate phase-3-4 --mode fresh-session|compact` records `awaiting-fresh-session` / `awaiting-compact` with `gate_selected` while implementation remains blocked, and the resumed/fresh session workflow then unblocks by running `approve-gate` with the selected mode. `same-context` remains a convenience path that selects and approves immediately. `check-phase-gates` now blocks every non-approved gate status, `SessionStart` injects a concise `WORKFLOW_RUNTIME_GATE` resume notice for awaiting states using stdin `cwd` before `CLAUDE_PROJECT_DIR`, and `/new-feature` + complex `/fix-bug` explicitly forbid running fresh-session approval in the leaving session. Bash/PowerShell controllers, fixtures, reference docs, and runtime tests cover mismatch rejection, awaiting-state blocking, selected-at preservation, cwd-based resume notice lookup, and same-context immediate approval.

**Stop worktrees from missing freshly upgraded Forge machinery.** `setup.sh` and `setup.ps1` now emit `FORGE_UPGRADE_UNCOMMITTED` when Forge-managed surfaces are modified or untracked after install/upgrade (`.claude/`, `.mcp.json`, `docs/reference/`, `docs/adr/`, `tests/e2e/`, `docs/ci-templates/`; excluding `.claude/local/**` and `.claude/settings.bak.*`). The warning is advisory and tells users to commit and push because new worktrees will not inherit uncommitted/unpushed machinery. `/new-feature` and `/fix-bug` now run a byte-identical preflight before creating a worktree; if parent-checkout Forge machinery is dirty, they fail closed with `FORGE_UPGRADE_UNCOMMITTED`, and if local `<default>` has unpushed Forge-managed commits while the workflow would base from `origin/<default>`, they fail closed with `FORGE_UPGRADE_UNPUSHED`. This prevents the dogfood/downstream failure where `setup.sh --upgrade` created new `.claude/` hooks/controllers, but the next workflow worktree was based on `origin/<default>` and lacked them. Tests cover setup warning output, command-block parity, dirty blocking, unpushed-default blocking, and `.claude/local/**` exclusion.

**Dogfood validation:** `forge-empty` ran `/new-feature note-count-command` from upgraded Forge runtime machinery. Choosing `fresh-session` left `phase-3-4` in `awaiting-fresh-session` with a `gate_selected` event and `approved_at=null`; attempted `Edit` and implementation `Bash` before approval both failed closed with `PHASE_GATE_PENDING`; the fresh worktree session received `WORKFLOW_RUNTIME_GATE` from `SessionStart`, read the Implementation Handoff, approved with `approve-gate --mode fresh-session`, then implemented successfully. The feature passed 234 pytest tests, ruff, mypy strict, code review, and E2E; implementation-session context peaked at 119,896 tokens versus the planning/gate session's 197,309.

## 5.57 — 2026-06-15

**Runtime phase-gate controller for the Phase 3→4 seam.** Adds a narrow Archon-inspired workflow runtime without introducing a full DAG engine: `.claude/local/workflow-run.json` stores gate state, `.claude/local/workflow-events.jsonl` records `gate_opened` / `gate_approved`, and paired controllers `hooks/lib/forge-workflow.{sh,ps1}` expose `status`, `open-gate`, `approve-gate`, and `check-tool`. New `check-phase-gates.{sh,ps1}` PreToolUse hooks register for `Bash|Edit|Write`; when `phase-3-4` is pending they allow read/status commands and `.claude/local/**` state updates but block implementation Bash/Edit/Write with `PHASE_GATE_PENDING: phase-3-4`. `/new-feature` and complex `/fix-bug` now open the gate after writing the Implementation Handoff and plan-review evidence, then stop until the user chooses `same-context`, `compact`, or `fresh-session`. Reference docs and tests cover controller idempotency, approval mode validation, hook block/allow behavior, settings/install wiring, and Bash/PowerShell parity surfaces.
