...
**Review certification: scoped evidence + convergence breaker — stop full-review spirals after a clean branch rebinds.** Field incident (msai-v2 PR #89): a branch certified clean by BOTH engines (15 iterations) and authorized for merge hit a docs/CHANGELOG-only merge conflict; the resolution commit moved HEAD, the head-rebind rule re-opened review at FULL scope, and stochastic reviewers ground from iteration 15 to 25 without converging — half a day for a docs rebind, with no stopping rule. A 5-advisor `/council` (Codex chairman) overruled prose-only guidance: the fix is a **machine-visible evidence contract**. **(1) Certification model:** the first both-engines-clean pass at `scope=full` is _certification_; a clean evidence line now means "review debt since the last clean evidence is discharged at HEAD," not "clean at HEAD." Evidence grammar becomes a coherent PAIR (`— codex|pr-toolkit clean — scope=full|delta — base=\`<sha>\` — head=\`<sha>\``, same scope/base/head) plus single-line forms (`mechanical re-stamp`, `codex deep-pass clean`, human adjudication); legacy scope-less pairs certify but never rebind. **(2) Scoped re-reviews:** after certification, a HEAD move is classified by the new shared helper `hooks/lib/review-scope.sh`+ dual-mode`.ps1` twin (`Invoke-ReviewScope`— dot-sourceable so hooks never spawn`pwsh`): the effective **PR-owned delta** via whole-diff + per-file `git patch-id --stable` against the merge-base (`--no-renames`; line-safe paths; quoted paths fail closed), plus the upstream interaction surface (merged-in AND still-pending default-branch movement). Docs-only/empty → a recorded **mechanical re-stamp**, no reviewer runs; code → BOTH engines review only the PR-owned delta (`codex exec review --base <last-clean-head>`; pr-toolkit gets a pinned scope statement; first-parent commit list INCLUDING merges + `--remerge-diff`so conflict resolutions stay in scope; upstream commits are interaction-analysis input, never findings targets). **(3) Recompute-don't-trust:**`check-workflow-gates.{sh,ps1}`and`build-evidence.{sh,ps1}` re-validate every chain-dependent claim with the helper (`--before N`) — fabricated mechanical-over-code, stale-base deltas, mixed-scope pairs, wrong merge-base full rows, post-amend forgeries, `scope=fullish`lookalikes, count-less N/A counter-erasure, and stale`## Workflow Archive`sections are all blocked or ignored (rebase / detached HEAD / git error → fail-closed full). **(4) Convergence breaker:**`POST_CERT_REVIEW_ROUND_LIMIT=3`(canonical in the helper; the loop-counter line makes finding-rounds countable) — past it, ship is hook-blocked (the block runs BEFORE the docs-only commit carve-out and outside the PASS branch, so neither N/A escapes nor docs commits bypass it) until a HUMAN records the head-bound`Post-certification tail adjudicated by human`line; in`/goal`the breaker halts for the human —`/council`is never the risk-acceptor (the`/goal`command strings + workflow rule carry the explicit carve-out). The oscillation trigger (new-P0/P1-in-own-delta) is prose-discipline with the round limit as the machine backstop — finding severity isn't recomputable from`state.md`(PRD US-003 amended; ADR 0009). **(5) Deep pass (default ON):** at certification, ONE deliberate Codex NATIVE`review --base <default>`adversarial audit; repos narrow via the new optional`## Review Configuration` (`deep_pass: high-impact-only`) in `CLAUDE.template.md`. Boundary (recorded): `SCOPE_REQUIRED`binds rebind events — an unchanged certified head is NOT ship-blocked by later upstream drift (pre-existing behavior, deliberately unchanged). Surfaces: NEW`hooks/lib/review-scope.sh/.ps1`(shipped by both installers + mapping rows), gate + evidence hook pairs,`rules/workflow.md`, both workflow commands, `commands/codex.md`, `state.template.md`, `CLAUDE.template.md`, ADR 0009. Tests: NEW `tests/template/test-review-scope.sh`(Level-1 helper cases + Level-2 gate cases, scratch-repo fixtures incl. master-default, spaced paths, delayed poisoning), 15 new`test-build-evidence.sh`cases (breaker suppresses`pr_ready` until head-bound adjudication), installer/lint coverage, ~119 new contract pins (incl. an unlabeled-legacy-grammar scan across docs AND hook diagnostics). Codex plan-review: **15 iterations to CLEAN** — chain poisoning, breaker bypasses, pair coherence, rename laundering, PS dual-mode, merge-resolution scope, stale refs, plus one P1 rebutted-with-spec-fix and accepted NOT MAINTAINED — with the helper live-smoked across 20+ scratch-repo scenarios during planning. FIX-3 (P2-on-certified-code severity policy) stays deferred per v5.50.
