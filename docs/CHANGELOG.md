...
**Field hits (actbl-he `/goal` run):** two autonomous stalls, both a Bash **write** under `.claude/local/`. First the agent ran `mkdir -p .claude/local/investigate` (from `/codex` Investigate mode); then it improvised `: > .claude/local/investigate/finding.txt` (a redirect-truncate). Claude Code never auto-approves writes under `.claude/` (ADR 0006 / Anthropic docs: protected path, only `bypassPermissions` skips it), so each tripped the sensitive-file prompt and — with no human in the loop — silently hung the run. v5.56 fixed the **read** side of this class; this is the **write** twin. The `auto-approve-local-writes` hook can't help (it intercepts only `Write|Edit`, never Bash), and a Bash command exposes no `file_path` to match.
...
3. **Runtime guardrail — `check-bash-safety.{sh,ps1}` check #9** blocks a Bash write-primitive (`mkdir|touch|cp|mv|tee|ln|install|dd|rmdir|rm|truncate|rsync`), `g?sed -i`, or a redirect (`([0-9]*|&)?>>?[&|]?`, incl. `>&`/`>|`) targeting `.claude/local/` (the dir itself or a subpath), and points back to the Write tool. Boundaries recognize shell separators (`;&|()`); the redirect uses a `(^|[^-])` anchor so an attached `echo hi>.claude/local/x` is caught while a `-> .claude/local` arrow in quoted data is not, and the target is scoped to the single redirect token so `> /tmp/log … .claude/local/x` (real target `/tmp`) is not a false positive; line-continuations are collapsed first. Framed as a **targeted guardrail for the common inline form**, not a parser — accepted residuals (write-text inside quoted data; interpreter-level writes) are documented in the hook.

Tests: `tests/template/test-bash-safety.sh` (61 assertions) gains a shared `NINE_BLOCK`/`NINE_ALLOW` corpus (looped against **both** hooks) covering both field bugs, separator boundaries, `>&`/`>|`, combined `sed -ni`, the dir itself, `truncate`/`rsync`/`gsed`/`chmod`, and backslash-continuation, plus allow-cases for the sanctioned codex launch, `/tmp` redirects, `;`-composition, and the `->`-in-quotes case. `test-contracts.sh` adds a region-scoped `.sh`↔`.ps1` parity contract (so a dropped `.ps1` branch fails even with no pwsh) and a "codex.md Investigate has no inline `.claude/` reader/writer" contract with reader/writer self-tests. Two-engine code review (Codex + PR-toolkit); full 13-suite `run-all.sh` green. Reaches downstream via `setup.sh --upgrade`.

## 5.56 — 2026-07-12 · Stop `/goal` stalling on a Bash-read of `state.md`; skip prettier on markdown

**Field hit (actbl-he `/goal` run):** while applying a council verdict, the agent improvised `sed -n '/## Workflow/,/## Update Rules/p' .claude/local/state.md | head -120` to peek at its own status file. Bash is **not** a read-only tool in Claude Code, so that read tripped CC's sensitive-file access prompt ("…always allow access to local/…") — and in an autonomous `/goal` loop with no human to answer, the run silently hung. The `auto-approve-local-writes` hook can't help: it only intercepts `Write|Edit`, never Bash. The Read tool would have been prompt-free (confirmed against CC docs via the claude-code-guide agent).
