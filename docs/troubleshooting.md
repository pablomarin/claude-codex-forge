...
## Codex-backed `/opinion` or `/council` returns empty output
...
4. **Diagnose with the bypass env var:** if you suspect upstream has fixed the bug or want to compare behavior:
   ...
   # Then run /opinion from the active host.
   ```
...
5. **Cancellation note:** Ctrl-C should terminate a Codex-backed `/opinion` or `/council` call cleanly. If Codex processes linger, check `ps -axo pid,command | grep codex-pty-helper` and report the version + reproducer.
