...
| `-f`, `--force`                    | Refresh managed files in an existing v6 install; refuses v5/mixed legacy machinery and points to the full-refresh preview                                                                                                                                                                                             |
| `-F`, `--full-refresh`             | Authoritative v5/mixed → v6 migration: reconcile canonical `.forge/` content and generated host adapters with ownership checks                                                                                                                                                                                        |
| `--dry-run`                        | With `-F`, run complete discovery and staging validation without writing target files; rerun without this flag only after `UPGRADE: READY`                                                                                                                                                                             |
