...
| `PreToolUse`     | Before Bash/Edit/Write                    | `check-phase-gates` blocks implementation mutations while a runtime phase gate is pending. Bash hooks also log commands, block dangerous patterns (pipe-to-shell, etc.), and `check-workflow-gates.sh` blocks ship actions from incomplete workflows. | Project          |
