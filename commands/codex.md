...
| Scoped delta review (post-certification) | `--base <last-clean-head-ref>` + `-c developer_instructions="<scope statement>"` (see scope statement below) |

**Scoped delta review (post-certification):** after the code-review loop's first clean pass (**certification**), re-reviews are scoped to the PR-owned delta since the last-clean-head, not the whole branch. Pass `--base <last-clean-head-ref>` (the last-clean-head SHA, or a `cert/<short-sha>` branch ref if Codex rejects a raw SHA) and inject the canonical scope statement via `-c developer_instructions=`: `Scoped delta review: findings are limited to this branch's own changes since <last-clean-head> (the PR-owned delta — commits listed below); upstream commits merged from the default branch are NOT findings targets. Also assess those incoming default-branch commits against this branch's touchpoints — files, APIs, configs, schemas, permissions, runtime behavior, dependencies.` Include the branch's own first-parent commit list (from `git log --first-parent <last-clean-head>..HEAD`, plus `git show --remerge-diff <merge-sha>` for any merge commit) in the instructions. See `rules/workflow.md` "Certification + scoped re-reviews" for the full dispatch model.
...
| Scoped delta review (post-certification) | `: > /tmp/codex_response.txt; .claude/hooks/lib/codex-pty.sh exec review --ephemeral --output-last-message /tmp/codex_response.txt --base <last-clean-head-ref> -c developer_instructions="Scoped delta review: findings are limited to this branch's own changes since <last-clean-head> (the PR-owned delta — commits listed below); upstream commits merged from the default branch are NOT findings targets. Also assess those incoming default-branch commits against this branch's touchpoints — files, APIs, configs, schemas, permissions, runtime behavior, dependencies." > /tmp/codex_response_full.txt 2>&1` |
