...
<p align="center">
  ...
  <a href="#version-history"><img alt="Version" src="https://img.shields.io/badge/version-5.28-blue?style=flat-square"></a>
...
| ------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5.28    | 2026-05-15 | `hooks/build-evidence.{sh,ps1}` — unified JSON evidence primitive for the `/forge-goal` autonomous loop (Layer 1). Parses `state.md`, queries git + gh + E2E mtime, computes `pr_ready` / `all_gates_green` / `progress_fingerprint`. Fixes ordering bug in `check-state-updated`: evidence now emits before the `stop_hook_active` early-return. |
