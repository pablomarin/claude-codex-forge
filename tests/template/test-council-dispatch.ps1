...
try {
  ...
  $failedFiles=@($attemptDirectories|Where-Object{$_.Name -like 'mixed-*'}|ForEach-Object{Get-ChildItem -LiteralPath $_.FullName -Recurse -File})
  Assert-True ($fallbackDirectories.Count -eq 1 -and $failedFiles.Count -eq 0) "PowerShell discards failed mixed artifacts (found: $($attemptDirectories.Name -join ','))"
